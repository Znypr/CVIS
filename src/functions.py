...
def show (images, more_images = None, title = None):
    ...
    for i in range(len(images)):
        if isinstance(type(images[i][0]), str):
            ...
            cv2.imshow(
                "Image {} from {}".format(i + 1, title), images[i]
            )
            cv2.resizeWindow(
                "Image {} from {}".format(
                    i + 1,
...
def save (images, format, folder):
    for i in range(1, len(images)):
        path = "../img/{}/{}_{}.{}".format(
            folder, folder, images[i][0], format
        )
...
def match_keypoints (
    ...
    if knn:
        matches = cv2.BFMatcher().knnMatch(descriptors[0],
            descriptors[1], k=2)
    else:
        matches = bf.match(descriptors[0], descriptors[1])

    if threshold:
        good = [[m] for m, n in matches if
                m.distance < threshold * n.distance]
        img = cv2.drawMatchesKnn(
            set[0], keypoints[0], set[1], keypoints[1], good, None
        )
        plt.imshow(img)
        plt.show()
        ...
        img = cv2.drawMatches(
            ...
            keypoints[0],
            set[1],
            keypoints[1], matches[:amount_matches], set[1], flags = 2
        )
        plt.imshow(img)
        plt.show()
...
def to_sift (set):
    ...
    for i in range(len(set)):
        ...
        keypoint, descriptor = siftobject.detectAndCompute(
            set[i], None
        )
